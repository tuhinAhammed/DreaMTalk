            ...
            <div className="innerList w-full h-[95%] overflow-y-auto  ">
            {
                userDataList.map((item) =>(
                        ...
                        <div className='flex justify-between items-center border-b-[1px] border-[#BFBFBF] py-[12px]'>
                            ...
                            {
                                sendFriendRequestData.includes(userData.uid+item.userId) || sendFriendRequestData.includes(item.userId+userData.uid) ? <button onClick={() => sendFriendRequest(item)} className="join px-[8px] py-[8px] bg-primary rounded-[5px] text-[#fff] text-[18px] font-semibold">Pending</button> : 
